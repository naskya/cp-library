...
data:
  ...
  bundledCode: "#line 1 \"include/combinatorics/factorial.hpp\"\n\n//! @file factorial.hpp\n\
    ...
    [[nodiscard]] std::array<Modint, Max + 1> factorial_modinv_array(const Modint\
    \ fact) {\n  std::array<Modint, Max + 1> res;\n  res[Max] = fact.inv();\n\n  for\
    \ (std::size_t i = Max; i > 0; --i)\n    res[i - 1] = res[i] * i;\n\n  return\
    ...
    \ typename Tp>\n[[nodiscard]] Modint combination(const Tp n, const Tp r, const\
    ...
    \ Tp, typename... Ts>\n[[nodiscard]] Modint multinomial(const Tp n, const Ts...\
    ...
    \ complexity: O(size(r))\ntemplate <std::size_t Size, typename Modint, typename\
    \ Tp, typename Container>\n[[nodiscard]] Modint multinomial(const Tp n, const\
    \ Container& r, const std::array<Modint, Size>& factorial_array, const std::array<Modint,\
    \ Size>& factorial_modinv_array) {\n  using Elem = std::decay_t<decltype(*std::cbegin(r))>;\n\
    \  if (n == 0)\n    CP_LIBRARY_WARN(\"n is zero.\");\n  if (n < 0) {\n    CP_LIBRARY_WARN(\"\
    n is negative.\");\n    return 0;\n  }\n  if (std::any_of(std::cbegin(r), std::cend(r),\
    \ [](const auto v) { return v < 0; })) {\n    CP_LIBRARY_WARN(\"r contains negative\
    ...
    \ Modint, typename Tp>\n[[nodiscard]] Modint stars_and_bars(const Tp n, const\
    \ Tp r, const std::array<Modint, Size>& factorial_array, const std::array<Modint,\
